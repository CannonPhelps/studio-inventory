<script lang="ts">
	...
	const isAuthRoute = $derived(['/login', '/onboarding', '/forgot-password', '/reset-password'].includes(currentPath));
...
<!-- For auth routes, use simple layout -->
{#if isAuthRoute}
...
{:else}
	...
	<div class="flex min-h-screen pt-16 lg:pt-0">
		...
		<div class="fixed top-0 left-0 right-0 z-40 bg-gradient-to-r from-accent to-accent-secondary border-b border-accent/20 shadow-custom-lg lg:hidden">
			<div class="flex items-center justify-between px-4 py-3">
				<div class="flex items-center space-x-4">
					...
					<div class="flex items-center space-x-3">
						...
						<h1 class="text-white font-bold text-lg">Studio Inventory</h1>
					</div>
				</div>
						...
						<div class="relative">
							...
							>
								<div class="w-8 h-8 bg-gradient-to-br from-white to-white/80 rounded-full flex items-center justify-center shadow-custom">
									<span class="text-sm font-semibold text-accent">{user.name.charAt(0)}</span>
								</div>
								<svg class="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
								</svg>
							</Button>
							<div id="userMenu" class="hidden absolute right-0 mt-2 w-48 bg-card border border-card rounded-xl shadow-custom-lg z-50 backdrop-blur-sm">
								<div class="p-3 border-b border-card">
									<div class="flex items-center space-x-3">
										<div class="w-10 h-10 bg-gradient-to-br from-accent to-accent-secondary rounded-full flex items-center justify-center">
								...
								<div class="p-2">
									<Button variant="ghost" size="sm" className="w-full text-left px-3 py-2 text-sm hover:bg-tertiary" on:click={() => window.location.href = '/settings'}>
									...
									{#if user.role === 'admin'}
										<Button variant="ghost" size="sm" className="w-full text-left px-3 py-2 text-sm hover:bg-tertiary" on:click={() => window.location.href = '/admin'}>
											<svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
